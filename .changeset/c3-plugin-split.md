...
'@file-preview/plugin-archive': minor
---

**C3 分包落地（方案 §9 多包结构）**：pdf / office / archive 三类重插件自 `@file-preview/core` 拆出为独立按需包——`@file-preview/plugin-pdf`（pdfjs-dist ^4.10.38）、`@file-preview/plugin-office`（mammoth ^1.9.0 / exceljs ^4.4.0，docx·xlsx·pptx）、`@file-preview/plugin-archive`（fflate ^0.8.2，zip + 炸弹四阈值防御）。
