/**
 * 全能力离线浏览器入口（G-Offline）：
 * - 内联全部依赖：core 七轻插件 + pdf/office/archive 三插件 + markdown-it/papaparse/
 *   dompurify/fast-xml-parser/fflate/emailjs-mime-parser/mammoth/exceljs/pdfjs(含 worker)/mediainfo(wasm)
 * - pdfjs 主线程模式：globalThis.pdfjsWorker 注入 WorkerMessageHandler，无需 Worker 文件，file:// 可用
 * - mediainfo WASM 以 base64 内联 → Blob URL，并包装默认 env 的资产解析与 loadWasm（免 fetch）
...
import mediainfoWasmB64 from 'mediainfo.js/dist/MediaInfoModule.wasm';
import type { EnvAdapter, EnvOptions } from '@file-preview/core';

// ── pdfjs 主线程模式（必须在首次使用 pdfjs 前注入）──
(globalThis as any).pdfjsWorker = pdfjsWorkerNS;

// ── mediainfo WASM：base64 → Blob URL（一次性）──
const WASM_BYTES = Uint8Array.from(atob(mediainfoWasmB64), (c) => c.charCodeAt(0));
const WASM_BLOB_URL =
...
/** 为环境注入离线资产：mediainfo.wasm 资产地址 + 免 fetch 的 loadWasm 直读。 */
function withOfflineAssets(env: EnvAdapter): EnvAdapter {
  const origAsset = env.getAssetUrl?.bind(env);
  const origLoadWasm = env.loadWasm.bind(env);
  return {
    ...env,
    getAssetUrl(name: string): string | undefined {
      if (name === 'mediainfo.wasm') return WASM_BLOB_URL || origAsset?.(name);
      return origAsset?.(name);
    },
    async loadWasm(url: string): Promise<ArrayBuffer | WebAssembly.Module> {
      if (url === WASM_BLOB_URL) {
        wasmBytesCache ??= WASM_BYTES.slice().buffer as ArrayBuffer;
        return wasmBytesCache;
      }
      return origLoadWasm(url);
    },
  };
}

/** 包装后的环境工厂：默认即具备离线 wasm 能力；调用方显式传入的配置优先。 */
export function createBrowserEnv(opts: EnvOptions = {}): EnvAdapter {
  return withOfflineAssets(core.createBrowserEnv(opts));
}
export function createNodeEnv(opts: EnvOptions = {}): EnvAdapter {
  return withOfflineAssets(core.createNodeEnv(opts));
}
...
/** 全量默认插件集（core 七轻插件 + 三拆分包）。 */
export function allPlugins() {
  return [...core.corePlugins(), pdfPlugin(), officePlugin(), zipPlugin()];
}
...
export const mediaPlugin = core.mediaPlugin;
